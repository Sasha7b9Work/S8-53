...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static const Choice mcShow;                             /// КУРСОРЫ - Показывать

static const Choice mcTrackingT1U1;                     ///< КУРСОРЫ - Курсоры T1,U1

static const Choice mcTrackingT2U2;                     ///< КУРСОРЫ - Курсоры T2,U2

static const Choice mcShowFreq;                         ///< КУРОСРЫ - 1/dT

static const Page mspSet;                               ///< КУРСОРЫ - УСТАНОВИТЬ
static void OnRotate_RegSet_Set(int angle);             ///< Вращение ручки УСТАНОВКА на странице КУРСОРЫ-УСТАНОВИТЬ
static const SmallButton sbSetExit;                     ///< КУРСОРЫ - УСТАНОВИТЬ - Выход
static void PressSB_Cursors_Exit(void);
static const SmallButton sbSetSource;                   ///< КУРСОРЫ - УСТАНОВИТЬ - Источник
...
static const SmallButton sbSetU;                        ///< КУРСОРЫ - УСТАНОВИТЬ - Курсоры U . Выбор курсора напряжения - курсор 1, курсор 2, оба курсора или отключены.
...
static const SmallButton sbSetT;                        ///< КУРСОРЫ - УСТАНОВИТЬ - Курсоры T . Выбор курсора времени - курсор 1, курсор 2, оба курсора или отключены.
...
static const SmallButton sbSet100;                      ///< КУРСОРЫ - УСТАНОВИТЬ - 100% . Установка 100 процентов в текущие места курсоров.
static void DrawSB_Cursors_100(int x, int y);
static void PressSB_Cursors_100(void);
static const SmallButton sbSetPointsPercents;           ///< КУРСОРЫ - УСТАНОВИТЬ - Перемещение . Переключение шага перемещения курсоров - по пикселям или по процентам.
...
{
    CircleIncreaseInt8((int8*)&CURS_CNTRL_U(chan), 0, 3);
}
...
{
    Channel source = CURS_SOURCE;
    CursCntrl cursCntrl = CURS_CNTRL_U(source);
...
static const SmallButton sbSetSource =
{
    ...
    {
        {
            DrawSB_Cursors_SourceA,
            "канал 1",
            "channel 1"
        },
        {
            DrawSB_Cursors_SourceB,
            "канал 2",
            "channel 2"
        }
    }
...
static void PressSB_Cursors_Source(void)
{
    Channel source = CURS_SOURCE_IS_A ? B : A;
    SetCursSource(source);
}

static void DrawSB_Cursors_Source(int x, int y)
{
    CURS_SOURCE_IS_A ? DrawSB_Cursors_SourceA(x, y) : DrawSB_Cursors_SourceB(x, y);
}
