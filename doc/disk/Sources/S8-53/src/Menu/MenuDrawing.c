...
static void DrawHintItem(int x, int y, int width)
{
    ...
    Painter_DrawStringInCenterRectAndBoundItC(x, y, width, 15, title, COLOR_BACK, COLOR_FILL);
    y = Painter_DrawTextInBoundedRectWithTransfers(x, y + 15, width, HINT(item), COLOR_BACK, COLOR_FILL);
    if (item->type == Item_SmallButton)
    {
...
void Menu_Draw()
{
    if(MenuIsShown() || TypeMenuItem(OpenedItem()) != Item_Page)
    {
        ...
        else
        {
            ...
            {
                ItemChoice_Draw(item, CalculateX(0), GRID_TOP, true);
            }
            else if(TypeMenuItem(item) == Item_Governor)
            {
                ItemGovernor_Draw(item, CalculateX(0), GRID_TOP, true);
            }
        }
    }
    ...
    if (gBF.showHelpHints)
    {
        ...
        if (gStringForHint)
        {
            Painter_DrawTextInBoundedRectWithTransfers(x, y, width, gStringForHint, COLOR_BACK, COLOR_FILL);
        }
        else if (gItemHint)
        {
...
void DrawTitlePage(Page *page, int layer, int yTop)
{
    ...
    Painter_FillRegionC(x - 1, yTop, MP_TITLE_WIDTH + 2, height + 2, COLOR_BACK);
    ...
    {
        Painter_DrawVolumeButton(x + 1, yTop + 1, MP_TITLE_WIDTH - 1, MP_TITLE_HEIGHT - 1, 3, ColorMenuTitle(false), ColorMenuTitleBrighter(), ColorMenuTitleLessBright(), shade, false);
    }
...
{
    ItemIPaddress_Draw(item, x, y, false);
}

static void DrawMACaddress(void* item, int x, int y)
{
    ItemMACaddress_Draw(item, x, y, false);
}

static void DrawFormula(void* item, int x, int y)
{
    ItemFormula_Draw(item, x, y, false);
}
...
{
    ItemSmallButton_Draw(item, LEFT_SB, y + 7);
}
...
void DrawOpenedPage(Page *page, int layer, int yTop)
{
    ...
    if (CurrentItemIsOpened(GetNamePage(page)))
    {
        ...
        {
            ItemIPaddress_Draw(item, CalculateX(1), ItemOpenedPosY(item), true);
        }
        else if (type == Item_MAC)
        {
            ItemMACaddress_Draw(item, CalculateX(1), ItemOpenedPosY(item), true);
        }
    }
