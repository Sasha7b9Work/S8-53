...
void Measure_DrawPageChoice()
{
    ...
    int x = num61or62 ? (GridRight() - 3 * GRID_WIDTH / 5) : GridLeft();
    int y = GRID_TOP;
    int dX = GRID_WIDTH / 5;
    int dY = 22;
    ...
    for(int row = 0; row < maxRow; row++)
    {
        for(int col = 0; col < maxCol; col++)
        {
            ...
            Painter_DrawRectangleC(x0, y0, dX, dY, COLOR_FILL);
            Painter_FillRegionC(x0 + 1, y0 + 1, dX - 2, dY - 2, active ? COLOR_FLASH_10 : COLOR_BACK);
            Painter_SetColor(active ? COLOR_FLASH_01 : COLOR_FILL);
            Painter_Draw10SymbolsInRect(x0 + 2, y0 + 1, Measure_GetChar(meas));
            if(meas < Measure_NumMeasures)
            {
                Painter_SetFont(TypeFont_5);
                Painter_DrawTextRelativelyRightC(x0 + dX, y0 + 12, measures[meas].name, active ? COLOR_FLASH_01 : COLOR_FILL);
                Painter_SetFont(TypeFont_UGO);
            }
            meas++;
        }
