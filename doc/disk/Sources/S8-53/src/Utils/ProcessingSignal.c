...
float CalculateVoltageMax(Channel chan)
{
    ...
    {
        markerHor[chan][0] = max;                           // «десь не округл€ем, потому что max может быть только целым
    }
...
float CalculateVoltageMin(Channel chan)
{
    ...
    {
        markerHor[chan][0] = min;                           // «десь не округл€ем, потому что min может быть только целым
    }
...
void CountedToCurrentSettings()
{
    ...
    if (dataSet->enableCh0 == 1 && (dataSet->range[0] != RANGE_A || dataSet->rShiftCh0 != RSHIFT_A))
    {
        Range range = RANGE_A;
    ...
    if (dataSet->enableCh1 == 1 && (dataSet->range[1] != RANGE_B || dataSet->rShiftCh1 != RSHIFT_B))
    {
        Range range = RANGE_B;
