...
const char* Tables_GetWarning(Warning warning)
{
    static const char *warnings[Warning_NumWarnings][2] =
    {
        ...
        {"ÂÊËŞ×ÅÍ ÏÈÊÎÂÛÉ ÄÅÒÅÊÒÎĞ",        "ENABLED PEAK. DET."},
        ...
        {"ÌÅÍŞ ÎÒËÀÄÊÀ ÂÊËŞ×ÅÍÎ",           "MENU DEBUG IS ENABLED"},
...
void DrawStr(int index, int x, int y)
{
    ...
    Painter_DrawTextC(x, y, symbolsAlphaBet[index], index == INDEX_SYMBOL ? COLOR_FLASH_01 : COLOR_FILL);
}
