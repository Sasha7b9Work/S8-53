...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
static void DrawGrid(int left, int top, int width, int height); // Нарисовать сетку.
static void DrawFullGrid(void);                                 // Нарисовать полную сетку.
static void DrawCursorsWindow(void);                    
static void DrawCursorRShift(Channel chan);                     // Нарисовать маркеры смещения по напряжению.
static void DrawCursorTrigLevel(void);                          // Нарисовать маркер уровня синхронизации.
static void DrawCursorTShift(void);                             // Нарисовать маркер смещения по времени.
static void DrawLowPart(void);                                  // Написать информацию под сеткой - в нижней части дисплея.
static void DrawHiRightPart(void);
static void DrawMath(void);
static void DrawSpectrum(void);
static void DrawTime(int x, int y);                             // Вывести текущее время.
...
static void DisableShowLevelRShift0(void);                      // Отключить вспомогательную линию маркера смещения по напряжению первого канала.
static void DisableShowLevelRShift1(void);                      // Отключить вспомогательную линию маркера смещения по напряжению второго канала.
static void DisableShowLevelTrigLev(void);
static void DrawConsole(void);                                  // Написать сообщения отладочной консоли.
static void DrawWarnings(void);                                 // Написать предупреждения.
static void OnRShiftMarkersAutoHide(void);
static void OnTimerShowWarning(void);
static void DrawTimeForFrame(uint timeMS);
static void DrawHorizontalCursor                                // Нарисовать горизонтальный курсор курсорных измерений.
...
static void DrawVerticalCursor                                  // Нарисовать вертикальный курсор курсорных измерений.
...
void DrawStringNavigation() 
{
    ...
    if(string) 
    {
        ...
        int height = 10;
        Painter_DrawRectangleC(GridLeft(), GRID_TOP, length + 2, height, COLOR_FILL);
        Painter_FillRegionC(GridLeft() + 1, GRID_TOP + 1, length, height - 2, COLOR_BACK);
        Painter_DrawTextC(GridLeft() + 2, GRID_TOP + 1, string, COLOR_FILL);
    }
}
...
void Display_RotateRShift(Channel chan)
{
    ...
    if(TIME_SHOW_LEVELS)
    {
        ...
        Timer_Enable((chan == A) ? kShowLevelRShift0 : kShowLevelRShift1, TIME_SHOW_LEVELS  * 1000, (chan == A) ? DisableShowLevelRShift0 :
                     DisableShowLevelRShift1);
    };
    Display_Redraw();
};
...
static void DrawMarkersForMeasure(float scale, Channel chan)
{
    ...
    Painter_SetColor(ColorCursors(chan));
    for(int numMarker = 0; numMarker < 2; numMarker++)
    {
        ...
        {
            Painter_DrawDashedHLine(GridFullBottom() - pos * scale, GridLeft(), GridRight(), 3, 2, 0);
        }

        pos = Processing_GetMarkerVertical(chan, numMarker);
        if (pos != ERROR_VALUE_INT && pos > 0 && pos < GridRight())
        {
            Painter_DrawDashedVLine(GridLeft() + pos * scale, GRID_TOP, GridFullBottom(), 3, 2, 0);
        }
    ...
    else
    {
        ...
        for (int i = startPoint; i < endPoint; i++)
        {
            ...
            if (x >= gridLeft && x <= gridRight)
            {
                ...
                Painter_DrawVLine((int)x, yMin, yMax);
            }
        }
    }
...
static void DrawSignalPointed(const uint8 *data, const DataSettings *ds, int startPoint, int endPoint, int minY, int maxY, float scaleY, float scaleX)
{
    ...
    else
    {
        for (int i = startPoint; i < endPoint; i++)
        {
            ...
            Painter_SetPoint(GridLeft() + index * scaleX, dat);
        }
    }
}
...
void DrawMath()
{
    if (DISABLED_DRAW_MATH || DS_GetData(A, 0) == 0 || DS_GetData(B, 0) == 0)
    {
        return;
    }
    ...
    Math_PointsVoltageToRel(dataAbs0, ds->length1channel, MATH_RANGE, MATH_RSHIFT, points);
    ...
    Painter_DrawRectangleC(GridLeft(), GridMathTop() + delta, WIDTH, HEIGHT, COLOR_FILL);
    Painter_FillRegionC(GridLeft() + 1, GridMathTop() + 1 + delta, WIDTH - 2, HEIGHT - 2, COLOR_BACK);
    Multiplier multiplier = MATH_MULTIPLIER;
    Painter_DrawTextC(GridLeft() + 2, GridMathTop() + 1 + delta, sChannel_Range2String(MATH_RANGE, multiplier), COLOR_FILL);
    Painter_DrawText(GridLeft() + 25, GridMathTop() + 1 + delta, ":");
    char buffer[20];
    Painter_DrawText(GridLeft() + 27, GridMathTop() + 1 + delta, sChannel_RShift2String(MATH_RSHIFT, MATH_RANGE, multiplier, buffer));
...
static void DrawSpectrumChannel(const float *spectrum, Color color)
{
    ...
    for (int i = 0; i < 256; i++) 
    {
        Painter_DrawVLine(gridLeft + i, gridBottom, gridBottom - gridHeight * spectrum[i]);
    }
}
...
static void WriteParametersFFT(Channel chan, float freq0, float density0, float freq1, float density1)
{
    ...
    Painter_SetColor(ColorChannel(chan));
    Painter_DrawText(x, y, SCALE_FFT_IS_LOG ? Float2Db(density0, 4, buffer) : Float2String(density0, false, 7, buffer));
    y += dY;
    Painter_DrawText(x, y, SCALE_FFT_IS_LOG ? Float2Db(density1, 4, buffer) : Float2String(density1, false, 7, buffer));
}
...
static void DRAW_SPECTRUM(const uint8 *data, int numPoints, Channel channel)
{
    ...
    if (!MenuIsShown() || MenuIsMinimize())
    {
        ...
        Painter_DrawRectangleC(FFT_POS_CURSOR_0 + GridLeft() - s, y0 - s, s * 2, s * 2, color);
        Painter_DrawRectangle(FFT_POS_CURSOR_1 + GridLeft() - s, y1 - s, s * 2, s * 2);

        Painter_DrawVLine(GridLeft() + FFT_POS_CURSOR_0, GridMathBottom(), y0 + s);
        Painter_DrawVLine(GridLeft() + FFT_POS_CURSOR_1, GridMathBottom(), y1 + s);
    }
}
...
void DrawSpectrum()
{
    ...
    Painter_DrawVLineC(GridRight(), GridChannelBottom() + 1, GridMathBottom() - 1, COLOR_BACK);
    ...
    Painter_DrawHLineC(GridChannelBottom(), GridLeft(), GridRight(), COLOR_FILL);
...
void DrawBothChannels(uint8 *data0, uint8 *data1)
{
    ...
    {
        DrawDataChannel(data0, A, gDSet, GRID_TOP, GridChannelBottom());
        DrawDataChannel(data1, B, gDSet, GRID_TOP, GridChannelBottom());
    }
    else
    {
        DrawDataChannel(data1, B, gDSet, GRID_TOP, GridChannelBottom());
...
void DrawDataMemInt()
{
    if(gDSmemInt != 0)
     {
        DrawDataChannel(gData0memInt, A, gDSmemInt, GRID_TOP, GridChannelBottom());
        DrawDataChannel(gData1memInt, B, gDSmemInt, GRID_TOP, GridChannelBottom());
    }
}
...
void DrawDataInModeWorkLatest()
{
    if (gDSmemLast != 0)
    {
        DrawDataChannel(gData0memLast, A, gDSmemLast, GRID_TOP, GridChannelBottom());
        DrawDataChannel(gData1memLast, B, gDSmemLast, GRID_TOP, GridChannelBottom());
    }
}
...
static void DrawDataInModePoint2Point()
{
    ...
    if (LAST_AFFECTED_CHANNEL_IS_B)
    {
        if (SELFRECORDER || !DS_NumElementsWithCurrentSettings())
        {
            DrawDataChannel(dataP2P_0, A, ds, GRID_TOP, GridChannelBottom());
            DrawDataChannel(dataP2P_1, B, ds, GRID_TOP, GridChannelBottom());
        }
        else
        {
            DrawDataChannel(data0, A, ds, GRID_TOP, GridChannelBottom());
            DrawDataChannel(data1, B, ds, GRID_TOP, GridChannelBottom());
        }
    }
    else
    {
...
bool DrawDataInModeNormal()
{
    ...
    else
    {
        ...
        {
            DrawBothChannels(DS_GetData(A, i), DS_GetData(B, i));
        }
    }
...
bool DrawData()
{
    ...
    Painter_DrawRectangleC(GridLeft(), GRID_TOP, GridWidth(), GridFullHeight(), COLOR_FILL);
...
#define NUM_POINTS (300 * 2)
	...
	if(width < 256)
    {
		Painter_DrawVLineArray(x, width, points, ColorChannel(chan));
	}
    else
    {
		Painter_DrawVLineArray(x, 255, points, ColorChannel(chan));
		Painter_DrawVLineArray(x + 255, width - 255, points + 255 * 2, ColorChannel(chan));
	}
}
...
void DrawMemoryWindow(void)
{
    ...
    Painter_DrawRectangleC(xVert0, top + (showFull ? 0 : 1), xVert1 - xVert0, bottom - top - (showFull ? 0 : 2), COLOR_FILL);
    ...
    {
        Painter_DrawVLineC(leftX - 2, top, bottom, COLOR_FILL);
        Painter_DrawVLine(rightX + 2, top, bottom);
        Painter_DrawHLine((bottom + top) / 2 - 3, leftX, xVert0 - 2);
        Painter_DrawHLine((bottom + top) / 2 + 3, leftX, xVert0 - 2);
        Painter_DrawHLine((bottom + top) / 2 + 3, xVert1 + 2, rightX);
        Painter_DrawHLine((bottom + top) / 2 - 3, xVert1 + 2, rightX);
    }
    ...
    // Маркер TPos
    Painter_FillRegionC(x0 - 3, 9, 6, 6, COLOR_BACK);
    Painter_DrawCharC(x0 - 3, 9, SYMBOL_TPOS_1, COLOR_FILL);
    ...
    Painter_FillRegionC(xShift - 1, 3, 6, 6, COLOR_BACK);
    Painter_FillRegionC(xShift, 4, 4, 4, COLOR_FILL);
    Painter_SetColor(COLOR_BACK);
    if(xShift < leftX - 2)
    {
        xShift = leftX - 2;
        Painter_DrawLine(xShift + 3, 5, xShift + 3, 7);
        Painter_DrawLine(xShift + 1, 6, xShift + 2, 6);
    }
    else if(xShift > rightX - 1)
    {
        xShift = rightX - 2;
        Painter_DrawLine(xShift + 1, 5, xShift + 1, 7);
        Painter_DrawLine(xShift + 2, 6, xShift + 3, 6);
    }
    else
    {
        Painter_DrawLine(xShift + 1, 5, xShift + 3, 5);
        Painter_DrawLine(xShift + 2, 6, xShift + 2, 7);
    }
}
...
void WriteCursors()
{
    ...
    if(sCursors_NecessaryDrawCursors())
    {
        Painter_DrawVLineC(x, 1, GRID_TOP - 2, COLOR_FILL);
        x += 3;
        ...
        if(!CURS_CNTRL_T_IS_DISABLE(source))
        {
            ...
            if(CURS_SHOWFREQ)
            {
                ...
                Painter_DrawRectangleC(x, GRID_TOP, width, 12, COLOR_FILL);
                Painter_FillRegionC(x + 1, GRID_TOP + 1, width - 2, 10, COLOR_BACK);
...
void DrawHiRightPart()
    ...
    if (!MODE_WORK_IS_LATEST)
    {
        ...
        if (gBF.trigEnable == 1)
        {
            Painter_FillRegion(x, 1 + y, GRID_TOP - 3, GRID_TOP - 7);
            Painter_DrawTextC(x + 3, 3 + y, set.common.lang == Russian ? "СИ" : "Tr", COLOR_BACK);
        }
    }
    ...
    if(!MODE_WORK_IS_DIRECT)
    {
        ...
        Painter_DrawVLineC(x, 1, GRID_TOP - 2, COLOR_FILL);
        x += 2;
        Painter_DrawText(set.common.lang == Russian ? x : x + 3, -1, set.common.lang == Russian ? "режим" : "mode");
        Painter_DrawStringInCenterRect(x + 1, 9, 25, 8, strings[MODE_WORK][set.common.lang]);
    }
    else
    {
        x -= 9;
    ...
    if (!MODE_WORK_IS_LATEST)
    {
        ...
        x += 2;
        y = 1;
        if (FPGA_CurrentStateWork() == StateWorkFPGA_Work)
        {
            Painter_Draw4SymbolsInRect(x, 1, SYMBOL_PLAY);
        }
        else if (FPGA_CurrentStateWork() == StateWorkFPGA_Stop)
        {
            Painter_FillRegion(x + 3, y + 3, 10, 10);
        }
        else if (FPGA_CurrentStateWork() == StateWorkFPGA_Wait)
        {
            int w = 4;
            int h = 14;
            int delta = 4;
...
void Display_Update()
{
    ...
    if (gMemory.needForSaveToFlashDrive == 1)
    {
        ...
        {
            Display_ShowWarningGood(FileIsSaved);
        }
...
void WriteValueTrigLevel()
{
    ...
    {
        float trigLev = RSHIFT_2_ABS(TRIG_LEVEL_SOURCE, RANGE(TRIG_SOURCE));     // WARN Здесь для внешней синхронизации неправильно рассчитывается уровень.
        ...
        int width = 96;
        int x = (GridWidth() - width) / 2 + GridLeft();
        int y = BottomMessages() - 20;
        Painter_DrawRectangleC(x, y, width, 10, COLOR_FILL);
        Painter_FillRegionC(x + 1, y + 1, width - 2, 8, COLOR_BACK);
        Painter_DrawTextC(x + 2, y + 1, buffer, COLOR_FILL);
    }
}
...
void DrawGridSpectrum()
{
    if (SCALE_FFT_IS_LOG)
    {
        ...
        {
            int y = GridMathTop() + i * scale;
            Painter_DrawHLineC(y, GridLeft(), GridLeft() + 256, ColorGrid());
            if (!MenuIsMinimize())
            {
                Painter_SetColor(COLOR_FILL);
    ...
    else if (SCALE_FFT_IS_LINEAR)
    {
        ...
        for (int i = 1; i < 5; i++)
        {
            ...
            if (!MenuIsMinimize())
            {
                Painter_DrawTextC(5, y - 4, strs[i], COLOR_FILL);
            }
        }
    }
    Painter_DrawVLineC(GridLeft() + 256, GridMathTop(), GridMathBottom(), COLOR_FILL);
}
...
void DrawFullGrid()
{
    if (sDisplay_IsSeparate())
    {
        ...
        Painter_DrawHLineC(GRID_TOP + GridFullHeight() / 2, GridLeft(), GridLeft() + GridWidth(), COLOR_FILL);
    }
    else
    {
...
static void DrawGridType1(int left, int top, int right, int bottom, float centerX, float centerY, float deltaX, float deltaY, float stepX, float stepY)
{
    ...
    Painter_DrawMultiVPointLine(17, top + stepY, masX, stepY, CalculateCountV(), ColorGrid());
    ...
    Painter_DrawMultiHPointLine(13, left + stepX, mas, stepX, CalculateCountH(), ColorGrid());
}
...
{ 
    ...
    masX[14] = right - 1;
    Painter_DrawMultiVPointLine(15, top + stepY, masX, stepY, CalculateCountV(), ColorGrid());
    ...
    mas[10] = bottom - 1;
    Painter_DrawMultiHPointLine(11, left + stepX, mas, stepX, CalculateCountH(), ColorGrid());
}
...
{
    Painter_DrawHPointLine(centerY, left + stepX, right, stepX);
    uint8 masY[6] = {top + 1, top + 2, centerY - 1, centerY + 1, bottom - 2, bottom - 1};
    Painter_DrawMultiHPointLine(6, left + deltaX, masY, deltaX, (right - top) / deltaX, ColorGrid());
    Painter_DrawVPointLine(centerX, top + stepY, bottom, stepY, ColorGrid());
    uint16 masX[6] = {left + 1, left + 2, centerX - 1, centerX + 1, right - 2, right - 1};
    Painter_DrawMultiVPointLine(6, top + deltaY, masX, deltaY, (bottom - top) / deltaY, ColorGrid());
...
void DrawScaleLine(int x, bool forTrigLev)
{
    ...
    for(int i = 0; i < 5; i++)
    {
        Painter_DrawLineC(x + 1, levels[i], x2 - 1, levels[i], COLOR_FILL);
    }
}
...
void DrawCursorTrigLevel()
{
    ...
    int trigLev = TRIG_LEVEL(chan) + ((chan == TrigSource_Ext) ? 0 : RSHIFT(chan) - RShiftZero);
    float scale = 1.0f / ((TrigLevMax - TrigLevMin) / 2 / GridChannelHeight());
    int y0 = (GRID_TOP + GridChannelBottom()) / 2 + scale * (TrigLevZero - TrigLevMin);
    ...
    Painter_DrawCharC(x + 5, y - 9 + dY, simbols[TRIG_SOURCE], COLOR_BACK);
    Painter_SetFont(TypeFont_8);
    ...
    if (gBF.drawRShiftMarkers == 1 && !MenuIsMinimize())
    {
        ...
        int shiftFull = TRIG_LEVEL_SOURCE + (TRIG_SOURCE_IS_EXT ? 0 : RSHIFT(chan));
        int yFull = GRID_TOP + DELTA + height - scale * (shiftFull - RShiftMin - TrigLevMin) - 4;
        Painter_FillRegionC(left + 2, yFull + 1, 4, 6, ColorTrig());
        Painter_SetFont(TypeFont_5);
        Painter_DrawCharC(left + 3, yFull - 5 + dY, simbols[TRIG_SOURCE], COLOR_BACK);
        Painter_SetFont(TypeFont_8);
    }
}
...
void DrawCursorRShift(Channel chan)
{
    ...
    if (chan == Math)
    {
        int rShift = MATH_RSHIFT;
        float scale = (float)GridMathHeight() / 960;
        float y = (GridMathTop() + GridMathBottom()) / 2 - scale * (rShift - RShiftZero);
        Painter_DrawCharC(x - 9, y - 4, SYMBOL_RSHIFT_NORMAL, COLOR_FILL);
        Painter_DrawCharC(x - 8, y - 5, 'm', COLOR_BACK);
        return;
    }
    if(!sChannel_Enabled(chan))
    {
    ...
    int rShift = RSHIFT(chan);
 
    float scale = (float)GridChannelHeight() / (STEP_RSHIFT * 200);
    float y = GridChannelCenterHeight() - scale * (rShift - RShiftZero);

    float scaleFull = (float)GridChannelHeight() / (RShiftMax - RShiftMin) * (sService_MathEnabled() ? 0.9f : 0.91f);
    float yFull = GridChannelCenterHeight() - scaleFull *(rShift - RShiftZero);

    if(y > GridChannelBottom())
    {
        Painter_DrawCharC(x - 7, GridChannelBottom() - 11, SYMBOL_RSHIFT_LOWER, ColorChannel(chan));
        Painter_SetPoint(x - 5, GridChannelBottom() - 2);
        y = GridChannelBottom() - 7;
        x++;
    }
    ...
    {
        Painter_DrawCharC(x - 7, GRID_TOP + 2, SYMBOL_RSHIFT_ABOVE, ColorChannel(chan));
        Painter_SetPoint(x - 5, GRID_TOP + 2);
        y = GRID_TOP + 7;
        x++;
    }
    ...
    {
        Painter_DrawCharC(x - 8, y - 4, SYMBOL_RSHIFT_NORMAL, ColorChannel(chan));
    ...
    if((!MenuIsMinimize() || !MenuIsShown()) && gBF.drawRShiftMarkers == 1)
    {
        Painter_FillRegionC(4, yFull - 3, 4, 6, ColorChannel(chan));
        Painter_DrawCharC(5, yFull - 9 + dY, chan == A ? '1' : '2', COLOR_BACK);
    }
    Painter_DrawCharC(x - 7, y - 9 + dY, chan == A ? '1' : '2', COLOR_BACK);
    Painter_SetFont(TypeFont_8);
}
...
void DrawCursorTShift()
{
    ...
    float scale = (lastPoint - firstPoint) / GridWidth();
    int gridLeft = GridLeft();
    int x = gridLeft + shiftTPos * scale - 3;
    if (IntInRange(x + 3, gridLeft, GridRight() + 1))
    {
        Painter_Draw2SymbolsC(x, GRID_TOP - 1, SYMBOL_TPOS_2, SYMBOL_TPOS_3, COLOR_BACK, COLOR_FILL);
    };
    ...
    if(IntInRange(shiftTShift, firstPoint, lastPoint))
    {
        ...
        Painter_Draw2SymbolsC(x, GRID_TOP - 1, SYMBOL_TSHIFT_NORM_1, SYMBOL_TSHIFT_NORM_2, COLOR_BACK, COLOR_FILL);
    }
    else if(shiftTShift < firstPoint)
    {
        Painter_Draw2SymbolsC(gridLeft + 1, GRID_TOP, SYMBOL_TSHIFT_LEFT_1, SYMBOL_TSHIFT_LEFT_2, COLOR_BACK, COLOR_FILL);
        Painter_DrawLineC(GridLeft() + 9, GRID_TOP + 1, GridLeft() + 9, GRID_TOP + 7, COLOR_BACK);
    }
    else if(shiftTShift > lastPoint)
    {
        Painter_Draw2SymbolsC(GridRight() - 8, GRID_TOP, SYMBOL_TSHIFT_RIGHT_1, SYMBOL_TSHIFT_RIGHT_2, COLOR_BACK, COLOR_FILL);
        Painter_DrawLineC(GridRight() - 9, GRID_TOP + 1, GridRight() - 9, GRID_TOP + 7, COLOR_BACK);
    }
}
...
void DrawCursors()
{
    ...
    if (sCursors_NecessaryDrawCursors())
    {
        ...
        if (bothCursors)
        {
            x0 = GridLeft() + CURS_POS_T0(source);
            x1 = GridLeft() + CURS_POS_T1(source);
...
void DrawMeasures()
{
    ...
    {
        int x0 = MEAS_POS_CUR_T0 - SHIFT_IN_MEMORY + GridLeft();
        int y0 = MEAS_POS_CUR_U0 + GRID_TOP;
        int x1 = MEAS_POS_CUR_T1 - SHIFT_IN_MEMORY + GridLeft();
        int y1 = MEAS_POS_CUR_U1 + GRID_TOP;
        SortInt(&x0, &x1);
        SortInt(&y0, &y1);
        Painter_DrawRectangleC(x0, y0, x1 - x0, y1 - y0, COLOR_FILL);
    }
    ...
    for(int str = 0; str < numRows; str++)
    {
        for(int elem = 0; elem < numCols; elem++)
        {
            ...
            {
                Painter_FillRegionC(x, y, dX, dY, COLOR_BACK);
                Painter_DrawRectangleC(x, y, dX, dY, COLOR_FILL);
                gBF.topMeasures = Math_MinFrom2Int(gBF.topMeasures, y);
            }
            if(active)
            {
                Painter_FillRegionC(x + 2, y + 2, dX - 4, dY - 4, COLOR_FILL);
            }
            if(meas != Measure_None)
            {
                char buffer[20];
                ...
                if(meas == MEAS_MARKED)
                {
                    Painter_FillRegionC(x + 1, y + 1, dX - 2, 9, active ? COLOR_BACK : COLOR_FILL);
                    Painter_DrawTextC(x + 4, y + 2, Measure_Name(str, elem), active ? COLOR_FILL : COLOR_BACK);
                }
                if(MEAS_SOURCE_IS_A)
                {
                    Painter_DrawTextC(x + 2, y + 11, Processing_GetStringMeasure(meas, A, buffer), ColorChannel(A));
                }
                else if(MEAS_SOURCE_IS_B)
                {
                    Painter_DrawTextC(x + 2, y + 11, Processing_GetStringMeasure(meas, B, buffer), ColorChannel(B));
                }
                else
                {
                    Painter_DrawTextC(x + 2, y + 11, Processing_GetStringMeasure(meas, A, buffer), ColorChannel(A));
                    Painter_DrawTextC(x + 2, y + 20, Processing_GetStringMeasure(meas, B, buffer), ColorChannel(B));
                }
            }
        }
    }
...
void WriteTextVoltage(Channel chan, int x, int y)
{
    ...
    bool inverse = INVERSE(chan);
    ModeCouple modeCouple = MODE_COUPLE(chan);
    Multiplier multiplier = MULTIPLIER(chan);
    Range range = RANGE(chan);
    uint rShift = RSHIFT(chan);
    bool enable = ENABLED(chan);
...
{
    char buffer[100];
    Painter_DrawTextC(x, y, text, COLOR_FILL);
    if(number == 0)
    {
        sprintf(buffer, "-");
    }
...
void DrawLowPart()
{
    ...
    Painter_DrawHLineC(GridChannelBottom(), 1, GridLeft() - Measure_GetDeltaGridLeft() - 2, COLOR_FILL);
    ...
    Painter_DrawVLineC(x + 95, GRID_BOTTOM + 2, SCREEN_HEIGHT - 2, COLOR_FILL);
    ...
    Painter_DrawVLineC(x + 79, GRID_BOTTOM + 2, SCREEN_HEIGHT - 2, COLOR_FILL);
    ...
    {
        Painter_Draw4SymbolsInRectC(x + 87, GRID_BOTTOM + 2, SYMBOL_ETHERNET, gBF.ethIsConnected ? COLOR_FILL : COLOR_FLASH_01);
    }

    if (gBF.connectToHost == 1 || gBF.cableVCPisConnected == 1)
    {
        Painter_Draw4SymbolsInRectC(x + 72, GRID_BOTTOM + 2, SYMBOL_USB, gBF.connectToHost ? COLOR_FILL : COLOR_FLASH_01);
    }
...
void DrawTimeForFrame(uint timeTicks)
{
    ...
    Painter_DrawRectangleC(GridLeft(), GridFullBottom() - 10, 84, 10, COLOR_FILL);
    Painter_FillRegionC(GridLeft() + 1, GridFullBottom() - 9, 82, 8, COLOR_BACK);
    Painter_DrawTextC(GridLeft() + 2, GridFullBottom() - 9, buffer, COLOR_FILL);
...
void Display_Clear()
{
    Painter_FillRegionC(0, 0, SCREEN_WIDTH - 1, SCREEN_HEIGHT - 2, COLOR_BACK);
}
...
void DrawConsole()
{
    ...
    for(int numString = firstString; numString <= lastString; numString++)
    {
        ...
        Painter_FillRegionC(GridLeft() + 1, GRID_TOP + 1 + count * (height + 1) + delta, width, height + 1, COLOR_BACK);
        int y = GRID_TOP + 5 + count * (height + 1) - 4;
        if(Font_GetSize() == 5)
        {
            y -= 3;
        }
        Painter_DrawTextC(GridLeft() + 2, y + dY + delta, strings[numString], COLOR_FILL);
        count++;
    }
...
//---------------------------------------------------------------------------------------------------------------------------------------------------
void Display_ClearFromWarnings()
{
...
{
    Painter_ResetFlash();
    Display_ShowWarn(Tables_GetWarning(warning));
    Sound_WarnBeepBad();
}
...
void DrawStringInRectangle(int x, int y, char const *text)
{
    ...
    int height = 8;
    Painter_DrawRectangleC(GridLeft(), y, width + 4, height + 4, COLOR_FILL);
    Painter_DrawRectangleC(GridLeft() + 1, y + 1, width + 2, height + 2, COLOR_BACK);
    Painter_FillRegionC(GridLeft() + 2, y + 2, width, height, COLOR_FLASH_10);
    Painter_DrawTextC(GridLeft() + 3, y + 2, text, COLOR_FLASH_01);
}
