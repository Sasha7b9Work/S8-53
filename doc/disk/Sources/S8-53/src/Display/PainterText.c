...
void Painter_LoadFont(TypeFont typeFont)
{
    ...
    Painter_SendToVCP(command, 2);
    Painter_SendToVCP((uint8*)(fonts[typeFont]), sizeof(Font));
}
...
#define SIZE_BUFFER 100
    ...
    *(command + 4) = length;
    ...
    Painter_SendToVCP(command, 1 + 2 + 1 + 1 + length);
    return retValue;
#undef SIZE_BUFFER
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
void Painter_DrawStringInCenterRectOnBackgroundC(int x, int y, int width, int height, const char *text, Color colorText, int widthBorder, 
...
void Painter_DrawHintsForSmallButton(int x, int y, int width, void *smallButton)
{
    ...
    Painter_FillRegionC(x, y, width, 239 - y, COLOR_BACK);
    Painter_DrawRectangleC(x, y, width, 239 - y, COLOR_FILL);
    ...
    while (structHelp->funcDrawUGO)
    {
        ...
        int yNew = Painter_DrawTextInRectWithTransfers(x + 23, y + 1, width - 30, 20, structHelp->helpUGO[set.common.lang]);
...
//----------------------------------------------------------------------------------------------------------------------------------------------------
void Painter_DrawTextInRect(int x, int y, int width, int height, char *text)
{
    int xStart = x;
